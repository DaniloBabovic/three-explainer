...
```javascript
<script setup>
	import { Color, DoubleSide, NoBlending, Vector3 }   from "three"
	import { onMounted } 								from 'vue'
	import { useExplainer, Origin, defaultOptions } 	from '@mapalchemy/explainer'
	import { Stage } 									from "@mapalchemy/explainer/src/explainerApi/scene/stage"
	...
	const insertExplainer = ( ) => {
		...
		// 3D Explainer APi 
		const container =           'three_container';  const showPlayer = true
		const stage = new Stage (   container,          showPlayer )
		const exp = useExplainer ( stage, container, true )
		...
		//HTML3D
		const html =`<iframe width="1000" height="500" frameBorder="0" src="https://www.youtube.com/embed/tgbNymZ7vqY?autoplay=1"> </iframe> `
		const css = {
			position: 'absolute',   color: "#FFFFFF",   background: 'black', padding: '0px',
			margin: '0px',          fontSize: '128px',  lineHeight: '128px', opacity: '0.1'
		}
		const material = {  transparent: true,  opacity: 0,         color: new Color ( '#000000' ), emissive:   new Color('#000000'),
							metalness:  0.5,    roughness:  0.5,    blending:   NoBlending,         side:       DoubleSide }
		const video = exp.add.html3d ( material, html, css, [ -6, -10, 0.1 ] )
		...
		// Animate 
		if ( exp.xGroup && exp.yGroup && exp.zGroup && plane.plane && video.meshLike && text3D.textMesh && sphere.sphereMesh && cube.cube && curveSin.curve && curveSin2.curve && line.line ) {
			...
			const t15= exp.fadeAfter        ( 'Video',    a1, 2, 0, video.meshLike,    { x: 0, y: -5, z: 4 },    { x: 4, y:2, z: 0 },   { from: 0, to: 1 }  )
			...
			const a67= exp.rotate           ( 'VR',       a4, 1, 0, video.meshLike,    { x: 0, y: 0, z: 0 },    { x: 0, y: 720, z: 0 }                       )
			...
			if ( print ) console.log( a25, a3, a6, a9, c1, c2, c3, t1, t15, t2, a65, a67 ) 
		}
		exp.animateManager.updatePlayer ( )
		exp.stage.rendererCSS.domElement.style.background = 'black'
		//exp.stage.rendererCSS.setClearColor( 0x000000, 0 );
		exp.stage.render ( )
	}
